...
func TestForwardsRoundTrip(t *testing.T) {
	...
	raw, err := Export(nil, hosts, rules, nil, nil).YAML()
	if err != nil {
		t.Fatal(err)
	}
	...
	p, err := Merge(d, nil, hosts, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	if len(p.Forwards) != 3 {
...
func TestImportingForwardsTwiceIsIdempotent(t *testing.T) {
	...
	d := Export(nil, hosts, rules, nil, nil)

	first, err := Merge(d, nil, hosts, nil, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	second, err := Merge(d, nil, hosts, first.Forwards, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestADocumentDeclaresTheVersionItNeeds(t *testing.T) {
	...
	if got := Export(nil, hosts, withRules, nil, nil).Version; got != 2 {
